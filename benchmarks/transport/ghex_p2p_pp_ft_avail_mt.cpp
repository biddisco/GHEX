...
// -----------------------------------------------------------------
// this used to be part of the thread local variables,
// but it was moved here to help with debugging lockups
struct msg_tracker {
...
#pragma omp master
...

    {
        ...
        //
        const int num_deb_msgs = 5;
        const int msecond = (1000*nsecs)/num_deb_msgs;
        //
...
#pragma omp barrier
            ...
            // -------------------------------
            // NB. these vars are thread local
            ...
            do {
                ...
                if (thread_id == 0) {
                    ...
                    if (now > dbg_time + std::chrono::milliseconds{msecond})
                    {
                        ...
                        // extra libfabric debugging msgs
                ...
                if (time_up) {
                    ...
                    if (thread_sends_complete) {
                        ...
                        if (thread_once_flag) {
                            ...
                            // only last thread to finish can trigger this (atomic latch)
...
//                            }
        ...
        // loop over receives : sanity check
        for (int j = 0; j < inflight*num_threads; j++)
        {
            ...
            else
            {
                ...
                throw std::runtime_error("All receive futures should be ready by this point");
            }
        }
